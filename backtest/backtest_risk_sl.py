...
class CombinedBacktest:
    ...
    def _precalculate_signals(self, max_steps=None, start_date=None, end_date=None):
        ...
        input_dim = len(engine.feature_names)
        risk_input_dim = input_dim + 3 + 1  # alpha features + 3 probs + 1 decision
        batch_size = 2048
        ...
        master_obs = np.zeros((feature_end - feature_start, num_assets, input_dim), dtype=np.float32)
        for i, asset in enumerate(self.env.assets):
            master_obs[:, i, :] = self.env.feature_engine.get_observation_vectorized(data_slice, asset)
        ...
        for asset_idx in range(num_assets):
            asset_obs = master_obs[:, asset_idx, :]
        ...
        if not self.verify_alpha:
            logger.info(f"Risk Inference (Batched {risk_input_dim}-features)...")
            ...
            for asset_idx in range(num_assets):
                ...
                # Create windows for risk_input_dim features
                windows = sliding_window_view(full_features, (self.seq_len, risk_input_dim)).squeeze(1)
...
def main():
    ...
    # Load Models
    # Alpha Model (LSTM - load from checkpoint for correct dimensions)
    engine = FeatureEngine()
    alpha_input_dim = len(engine.feature_names)
    checkpoint = torch.load(args.alpha_model, map_location=device)
    if "model_config" in checkpoint:
        alpha_model = AlphaSLModel(**checkpoint["model_config"]).to(device)
        alpha_model.load_state_dict(checkpoint["model_state_dict"])
    else:
        alpha_model = AlphaSLModel(input_dim=alpha_input_dim).to(device)
        alpha_model.load_state_dict(checkpoint)
    alpha_model.eval()

    # Risk Model (risk_input_dim features: alpha features + 3 probs + 1 decision)
    risk_input_dim = alpha_input_dim + 3 + 1
    risk_model = MultiHeadRiskLSTM(input_size=risk_input_dim, hidden_size=128, num_layers=2).to(device)
