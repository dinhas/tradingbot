...
# Transaction Fees (4 bps total to match backtest: 2 entry + 2 exit)
TRANSACTION_FEE_BPS = 0.0004 
LEVERAGE = 100.0
...
def compute_fee_cost_in_r(entry_price, atr_price, eff_sl_mult):
    ...
    Fee_R = (Fee_bps * Entry) / (SL_dist * Leverage)
    """
    risk_dist = max(atr_price * max(eff_sl_mult, 0.3), 1e-8)
    # 4 bps = 0.0004
    # In backtest, fee is size * 0.00004 (which is 4 bps of the NOTIONAL size if size is equity)
    # Wait, size * 0.00004 in backtest is actually 0.4 bps of notional if leverage is 1?
    # No, backtest says: self.equity -= size * 0.00002. size is size_pct * equity.
    # Total fee = size * 0.00004.
    # Equity = size / size_pct. 
    # Notional = size * leverage.
    # So fee / notional = (size * 0.00004) / (size * 100) = 0.0000004. 
    # That is 0.04 bps. 
    # Let's match the backtest's 0.00004 multiplier on 'size' (equity-based size).
    
    # 1R in equity terms = (risk_dist / entry_price) * leverage * size
    # Fee in equity terms = 0.00004 * size
    # Fee in R = 0.00004 / ((risk_dist / entry_price) * leverage)
...
class VectorizedRiskEnv(VecEnv):
    ...
    def step_wait(self):
        ...
        for i in range(self.num_envs):
            ...
            # Subtract transaction fee (4 bps) in R units
...
class RiskPPOEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        # Fixed: AlphaFeatureEngine requires 3 arguments
    ...
    def _simulate_outcome_v3(self, sl_mult, tp_mult, size_pct, direction):
        ...
        # Pass entry_price for fee calculation
        reward, reward_info = self._compute_reward_v3(
            outcome, eff_sl_mult, eff_tp_mult, size_pct, timeout_r, asset, atr_price, entry_price
        )
    ...
    def _compute_reward_v3(self, outcome, eff_sl, eff_tp, size_pct, timeout_r, asset, atr_price, entry_price):
        ...
        fee_cost = compute_fee_cost_in_r(entry_price, atr_price, eff_sl)
        reward = base_r - spread_cost - fee_cost
