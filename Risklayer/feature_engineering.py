...
class FeatureEngine:
    ...
    def calculate_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Normalize (Simple Robust Scaling)
        feature_df = (feature_df - feature_df.rolling(100).median()) / (feature_df.rolling(100).quantile(0.75) - feature_df.rolling(100).quantile(0.25) + 1e-6)
    ...
    def _get_asset_features(self, df: pd.DataFrame, asset: str) -> Dict[str, pd.Series]:
        ...
        f[f"{asset}_bb_pos"] = (close - bb.bollinger_lband()) / (bb.bollinger_hband() - bb.bollinger_lband() + 1e-6)
        ...
        # Pro Features (subset)
        # VWAP Dev (1)
        vwap = (close * volume).groupby(df.index.date).cumsum() / volume.groupby(df.index.date).cumsum()
        f[f"{asset}_vwap_dev"] = (close - vwap) / (atr_14 + 1e-6)

        # Delta Pressure (1)
        direction = np.sign(close - open_)
        f[f"{asset}_pressure"] = (volume * direction).rolling(20).sum() / (volume.rolling(20).sum() + 1e-6)

        # Vol Squeeze (1)
        f[f"{asset}_squeeze"] = (bb.bollinger_hband() - bb.bollinger_lband()) / (atr_14 + 1e-6)
    ...
    def get_observation_cols(self, asset: str) -> list:
        ...
        cols = [
            f"{asset}_ret1", f"{asset}_ret12", f"{asset}_atr_ratio", f"{asset}_bb_pos",
            f"{asset}_price_ema9", f"{asset}_ema9_ema21", f"{asset}_rsi", f"{asset}_macd_hist",
            f"{asset}_vol_ratio", f"{asset}_vwap_dev", f"{asset}_pressure", f"{asset}_squeeze",
            f"{asset}_wick_rej",
            'hour_sin', 'hour_cos', 'day_sin', 'day_cos',
            'session_asian', 'session_london', 'session_ny', 'session_overlap',
            'market_dispersion', 'risk_on_score'
        ]
