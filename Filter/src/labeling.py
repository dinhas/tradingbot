...
class Labeler:
    def __init__(self, tp_mult: float = 4.0, sl_mult: float = 2.0, ema_window: int = 100,
                 max_bars: int = 18, adx_threshold: float = 15.0, min_edge_r: float = 0.12,
                 min_bars_between: int = 4):
        """
        Trend-Following Triple Barrier Labeler.

        V6 Geometry: TP=4x ATR, SL=2x ATR (2:1 reward-to-risk).
        """
    ...
    def label_data(self, df: pd.DataFrame, asset: str) -> pd.DataFrame:
        ...
        raw_return = np.zeros(n, dtype=np.float32)
        long_net_r = np.zeros(n, dtype=np.float32)
        short_net_r = np.zeros(n, dtype=np.float32)
        long_raw_return = np.zeros(n, dtype=np.float32)
        short_raw_return = np.zeros(n, dtype=np.float32)
        ...
        for i in range(n - self.max_bars):
            ...
            long_raw_return[i] = (long_outcome.exit_price - long_outcome.entry_price) / long_outcome.entry_price
            short_raw_return[i] = (short_outcome.exit_price - short_outcome.entry_price) / short_outcome.entry_price
            ...
            if long_net_r[i] >= short_net_r[i]:
                ...
                raw_return[i] = long_raw_return[i]
            else:
                directions[i] = 0.0
                net_r[i] = short_net_r[i]
                raw_return[i] = short_raw_return[i]
        ...
        # Enforce minimum gap between labeled events to reduce label overlap.
        # De Prado's sequential bootstrap insight: overlapping labels from
        # nearby bars share the same price path and inflate sample count.
        if self.min_bars_between > 1:
            last_tradeable_idx = -self.min_bars_between - 1
            for i in range(n):
                if action_class[i] != 0.0:
                    if i - last_tradeable_idx < self.min_bars_between:
                        action_class[i] = 0.0  # demote to hold
                        tradeable[i] = 0.0
                    else:
                        last_tradeable_idx = i
        ...
        return pd.DataFrame({
            ...
            'long_raw_return': long_raw_return,
            'short_raw_return': short_raw_return,
