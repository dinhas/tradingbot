...
class CTraderAmendError(Exception):
    """Raised when cTrader rejects an amend SL/TP request."""
    def __init__(self, pos_id, error_code, description):
        self.pos_id = pos_id
        self.error_code = error_code
        self.description = description
        super().__init__(f"Amend rejected for position {pos_id}: [{error_code}] {description}")
...
class CTraderClient:
            ...
            for bar in event.trendbar:
                if bar.period == ProtoOATrendbarPeriod.M5:
                    ...
                    if last_ts is None or current_ts > last_ts:
                        self.logger.debug(f"M5 Candle closed for symbol {symbol_id} (TS: {current_ts})")
    ...
    def _send_heartbeat(self):
        ...
        d.addErrback(lambda f: self.logger.debug(f"Heartbeat failed: {f}"))
    ...
    def send_request(self, request, retries=3, timeout=10):
        ...
        for i in range(retries):
            try:
                ...
                last_error = e
                if i < retries - 1:
                    self.logger.debug(f"Request failed (attempt {i+1}/{retries}): {e}")
                    # Exponential backoff: 1s, 2s, 4s...
                    yield deferLater(reactor, 1.0 * (2**i), lambda: None)
                else:
                    self.logger.warning(f"Request failed after {retries} attempts: {e}")
    ...
    def subscribe_trendbars(self, symbol_ids, period=ProtoOATrendbarPeriod.M5):
        ...
        for symbol_id in symbol_ids:
            ...
            res_msg = yield self.send_request(req)
            self.logger.debug(f"Subscribed to M5 trendbars for symbol {symbol_id}")
    ...
    @inlineCallbacks
    def amend_position_sltp(self, pos_id, stop_loss=None, take_profit=None):
        """Amends Stop Loss and/or Take Profit for an existing open position.
        
        Raises:
            CTraderAmendError: If cTrader returns an error response with errorCode/description.
        """
        ...
        # Check 1: ProtoOAErrorRes / ProtoErrorRes (standard error response)
        ...
        if name in ('ProtoOAErrorRes', 'ProtoErrorRes'):
            ...
            error_desc = getattr(payload, 'description', 'No description')
            raise CTraderAmendError(pos_id, error_code, error_desc)

        # Check 2: ProtoOAExecutionEvent with errorCode set (amend rejected silently)
        if hasattr(payload, 'errorCode') and payload.errorCode:
            raise CTraderAmendError(pos_id, payload.errorCode, f"Amend rejected: {payload.errorCode}")
