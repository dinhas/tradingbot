...
class TelegramNotifier:
    ...
    async def _config_command(self, update, context):
        ...
        msg = (
            ...
            f"• Alpha confidence: `{self.orchestrator.config.get('ALPHA_CONFIDENCE_THRESHOLD', 0.60)}`\n"
            f"• Filter threshold: `{self.orchestrator.config.get('FILTER_THRESHOLD', 0.565)}`\n"
            f"• SL multiplier: `{self.orchestrator.config.get('SL_MULTIPLIER', 2.0)}x ATR`\n"
            f"• TP multiplier: `{self.orchestrator.config.get('TP_MULTIPLIER', 4.0)}x ATR`"
        )
        await update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN)
