...
MARKET_DATA_DIR = os.environ.get(
    "MARKET_DATA_DIR",
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data")),
)
PPO_DATASET_PATH = os.environ.get(
    "PPO_DATASET_PATH",
    os.path.join(os.path.dirname(__file__), "data", "ppo_risk_dataset.parquet"),
)
...
def train():
    logger.info(
        f"Starting Optimized PPO Risk Model Training on {'cuda' if torch.cuda.is_available() else 'cpu'}"
    )
    ...
    if not os.path.exists(PPO_DATASET_PATH):
        raise FileNotFoundError(
            f"Dataset not found at {PPO_DATASET_PATH}. Please run generate_rl_risk_dataset.py first."
        )
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=100_000, save_path=MODELS_DIR, name_prefix="ppo_risk_model_v2_opt"
    )
