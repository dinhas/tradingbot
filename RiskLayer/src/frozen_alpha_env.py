...
class TradingEnv(gym.Env):
    ...
    def _parse_action(self, action):
        ...
        for i, asset in enumerate(self.assets):
            if self.stage == 1:
                ...
                sl_raw = np.clip(
                    (action[base_idx + 2] + 1) / 2 * 2.5 + 0.5, 0.5, 3.0
                )  # 0.5 to 3.0
                tp_raw = np.clip(
                    (action[base_idx + 3] + 1) / 2 * 3.5 + 1.5, 1.5, 5.0
                )  # 1.5 to 5.0
                parsed[asset] = {
                    "direction": 1
                    if direction_raw > 0.33
    ...
    def _open_position(self, asset, direction, act, price, atr):
        ...
        # Add breathing room buffer to SL (prevents immediate SL hits from spread)
        sl_buffer = self.risk_config.breathing_room.calculate_sl_buffer(spread, atr)
        sl_dist_with_buffer = sl_dist + sl_buffer

        # Calculate SL/TP levels from entry price
    ...
    def _close_position(self, asset, price):
        ...
        # Spread cost in dollars: notional_value * (spread / price)
    ...
    def _update_positions(self):
        ...
        for asset, pos in list(self.positions.items()):
            ...
            if pos["direction"] == 1:  # Long
                # For SL check: we need bid price (lower)
                bid_price = price - spread / 2
                if bid_price <= pos["sl"]:
                    # SL hit - close at SL price (which already includes buffer)
                    self._close_position(asset, pos["sl"])
                elif price >= pos["tp"]:
                    # TP hit - close at TP price
                    self._close_position(asset, pos["tp"])
            else:  # Short
                # For SL check: we need ask price (higher)
