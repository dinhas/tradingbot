...
def generate_sl_dataset(
    model_path, data_dir, output_file, limit=None, max_samples=None
):
    # 1. Load Model
    ...
    for b_start in tqdm(batch_starts, desc="Processing Batches"):
        ...
        for asset in assets:
            ...
            obs_batch = env.feature_engine.get_observation_vectorized(
                batch_df_slice, asset
            )
        ...
        for asset in assets:
            ...
            for local_idx, global_idx in zip(active_local_indices, global_indices):
                ...
                # --- Labeling Logic with Spread-Aware SL ---
                # Get spread for this asset
                spread_profile = DEFAULT_RISK_CONFIG.spread_profiles.get(asset)
                if spread_profile:
                    # Use base spread for labeling (conservative estimate)
                    spread = spread_profile.base_spread_pips * spread_profile.pip_value
                else:
                    spread = entry_price * 0.0002  # Fallback: 2 pips
                ...
                # Calculate SL with breathing room buffer
                # Buffer = spread * 2 (for entry + exit) + ATR-based buffer
                sl_buffer = DEFAULT_RISK_CONFIG.breathing_room.calculate_sl_buffer(
                    spread, atr
                )
                sl_buffer_atr = sl_buffer / atr if atr > 0 else 0.2

                # Target SL includes MAE + buffer (conservative)
                target_sl_mult = np.clip(mae_atr + sl_buffer_atr, 0.3, 5.0)
                # Target TP is MFE (optimistic)
                target_tp_mult = np.clip(mfe_atr, 0.1, 10.0)

                # Confidence / Size
                rr_ratio = target_tp_mult / max(target_sl_mult, 1e-9)
                target_size = (
                    np.clip((mfe_atr - 0.5) / 2.0, 0.0, 1.0) if rr_ratio >= 1.5 else 0.0
                )
        ...
        if has_data:
            ...
            if os.path.exists(output_file):
                ...
                pd.concat([existing_df, batch_df], ignore_index=True).to_parquet(
    ...
    logger.info(
        f"Dataset Generation Complete. Saved {total_signals} samples to {output_file}"
    )
...
if __name__ == "__main__":
    ...
    generate_sl_dataset(
        args.model, args.data, args.output, args.limit, args.max_samples
    )
