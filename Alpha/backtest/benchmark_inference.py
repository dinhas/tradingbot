...
def benchmark_inference(model_path, data_dir, n_iterations=100):
    ...
    stage = 1
    if "stage_2" in model_path:
        stage = 2
    if "stage_3" in model_path:
        stage = 3
    ...
    for i in range(n_iterations):
        ...
        low_bound = min(50, max(0, len(env.processed_data) // 10))
        high_bound = max(low_bound + 1, len(env.processed_data) - 50)
        rand_step = np.random.randint(low_bound, high_bound)
