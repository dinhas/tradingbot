...
class TradeGuardFeatureBuilder:
    ...
    def get_multi_asset_obs(self, step, trade_infos, portfolio_state):
        ...
        for asset in self.assets:
            ...
            # Ensure safe access to nested dicts
            p_state = portfolio_state.get(asset, {})
            f_alpha = [
                p_state.get('action_raw', 0),
                p_state.get('signal_persistence', 0),
                p_state.get('signal_reversal', 0)
            ]
            ...
            # Safe access to OHLCV arrays
            if asset in self.ohlcv_arrays:
        ...
        # Use first asset's time index
        if self.assets[0] in self.df_dict:
        ...
        f_global = [
            ...
            1.0 if (13 <= hour <= 17) else 0.0, # NY/London Overlap
