...
2. Labeling produces the expected {tradeable, direction, net_r, valid} schema and a sane distribution.
...
from Alpha.src.model import AlphaSLModel, binary_signal_loss
...
def run_micro_validation():
    ...
    for batch_X, batch_actions, batch_assets in train_loader:
        ...
        loss = binary_signal_loss(outputs, batch_actions)
    ...
    with torch.no_grad():
        ...
        preds = (torch.sigmoid(outputs['action_logits'].squeeze(-1)) >= 0.5).long().numpy()
    targets = y_actions.astype(np.int64)
    cm = np.zeros((2, 2), dtype=np.int64)
    for t, p in zip(targets, preds):
        cm[t, p] += 1
    logger.info(f"\nTradeability confusion matrix (rows=true, cols=pred) [NoTrade, Trade]:\n{cm}")
