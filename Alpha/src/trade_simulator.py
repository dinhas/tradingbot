...
class TradeSimulator:
    ...
    def returns(self, entry_price: float, exit_price: float, direction: int,
                atr: float) -> tuple[float, float, float, float]:
        raw_return = (exit_price - entry_price) / entry_price
        gross_return = raw_return * direction * self.config.leverage
        ...
        return float(gross_return), float(net_return), float(net_r), float(raw_return)
    ...
    def simulate(self, opens: np.ndarray, highs: np.ndarray, lows: np.ndarray,
        ...
        gross_return, net_return, net_r, raw_return = self.returns(
