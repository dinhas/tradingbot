...
class Labeler:
    ...
    def label_data(self, df: pd.DataFrame, asset: str) -> pd.DataFrame:
        """
        Labels data using Triple Barrier Method filtered by 1H Trend.
        """
        ...
        n = len(df)
        # We need some padding at the end to avoid index errors during lookahead
        for i in range(n - 1):
            ...
            direction = 0 # Default: No barrier hit or Neutral
            ...
            if current_trend == 1: # Bullish Trend -> ONLY Look for BUYS
                ...
                # Look forward until a barrier is hit
                for j in range(i + 1, n):
                    if prices_high[j] >= upper_barrier:
                        direction = 1 # TP Hit
                        break
                    if prices_low[j] <= lower_barrier:
                        direction = 0 # SL Hit (Label 0 as it failed the trend trade)
                        break
