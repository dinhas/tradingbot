...
from shared_config import NORMALIZATION_WINDOW, FEATURE_LOOKBACKS
...
class FeatureEngine:
    ...
    def _define_feature_names(self):
        ...
        self.feature_names = [
            ...
            "htf_ema_alignment",
            "htf_rsi_divergence",
            "swing_structure_proximity",  # HTF (3)
            "vwap_deviation",
            "delta_pressure",
            "volume_shock",  # Volume (3)
            "volatility_squeeze",
            "wick_rejection_strength",
            "breakout_velocity",  # PA (3)
            "rsi_slope_divergence",
            "macd_momentum_quality",  # Momentum (2)
            # --- CROSS-ASSET (5) ---
            "corr_basket",
            "rel_strength",
            "corr_xauusd",
    ...
    def preprocess_data(self, data_dict):
        ...
        new_features_df = pd.DataFrame(all_new_cols, index=aligned_df.index).astype(
            np.float32
        )
        ...
        logger.info(
            f"Preprocessing complete. Total features: {len(normalized_df.columns)}"
        )
    ...
    def _get_technical_indicators(self, df, asset):
        ...
        new_cols[f"{asset}_bb_position"] = (close - bb.bollinger_lband()) / (
    ...
    def _calculate_pro_features(self, df, asset, technical_cols):
        ...
        close_1h = (
            close.resample("60min", label="right", closed="right")
            .last()
        ...
        rsi_1h = (
            RSIIndicator(close_1h, window=14).rsi().reindex(close.index, method="ffill")
        )
        ...
        new_features[f"{asset}_swing_structure_proximity"] = np.minimum(
            dist_high, dist_low
        )

        # B. VOLUME & ORDER FLOW
        vwap_series = (close * volume).groupby(df.index.date).cumsum() / volume.groupby(
            df.index.date
        ).cumsum()
        ...
        velocity = np.where(
            close > range_h.shift(1),
            (close - range_h.shift(1)) / atr,
        ...
        new_features[f"{asset}_rsi_slope_divergence"] = np.sign(price_slope) - np.sign(
            rsi_slope
        )

        macd_signal = MACD(close).macd_signal()
        hist_slope = macd_hist.diff()
        new_features[f"{asset}_macd_momentum_quality"] = hist_slope * np.sign(
            macd_signal
        )
    ...
    def _get_cross_asset_features(self, df, technical_cols):
        ...
        for asset in self.assets:
            asset_ret = technical_cols.get(
                f"{asset}_return_1", df.get(f"{asset}_return_1")
            )
        ...
        for a in self.assets:
            ret12_list.append(
                technical_cols.get(f"{a}_return_12", df.get(f"{a}_return_12"))
            )
    ...
    def _get_session_features(self, df, technical_cols):
        ...
        if atr_ratio_cols:
            new_cols["market_volatility"] = pd.concat(atr_ratio_cols, axis=1).mean(
                axis=1
            )
        else:
    ...
    def _normalize_features(self, df):
        ...
        for asset in self.assets:
            cols_to_normalize.extend(
                [
                    ...
                    # New Features
                    f"{asset}_htf_ema_alignment",
                    f"{asset}_htf_rsi_divergence",
                    f"{asset}_swing_structure_proximity",
                    f"{asset}_vwap_deviation",
                    f"{asset}_delta_pressure",
                    f"{asset}_volume_shock",
                    f"{asset}_volatility_squeeze",
                    f"{asset}_wick_rejection_strength",
                    f"{asset}_breakout_velocity",
                    f"{asset}_rsi_slope_divergence",
                    f"{asset}_macd_momentum_quality",
                ]
            )
        ...
        for col in cols_to_normalize:
            if col in df.columns:
                rolling_median = df[col].rolling(window=NORMALIZATION_WINDOW).median()
                rolling_q75 = (
                    df[col].rolling(window=NORMALIZATION_WINDOW).quantile(0.75)
                )
                rolling_q25 = (
                    df[col].rolling(window=NORMALIZATION_WINDOW).quantile(0.25)
                )
    ...
    def get_observation_vectorized(self, df, asset):
        ...
        obs_cols.extend(
            [
                f"{asset}_ema_9",
                f"{asset}_ema_21",
                f"{asset}_price_vs_ema9",
                f"{asset}_ema9_vs_ema21",
            ]
        )
        ...
        obs_cols.extend(
            [
                f"{asset}_corr_basket",
                f"{asset}_rel_strength",
                f"{asset}_corr_xauusd",
                f"{asset}_corr_eurusd",
                f"{asset}_rank",
            ]
        )
