...
def leakage_check() -> bool:
    ...
    loader_ld = DataLoader()
    macro_df = loader_ld.load_macro_data()
    cot_df = loader_ld.load_cot_data()
    aligned_full, norm_full = FeatureEngine().preprocess_data(
        {a: df.copy() for a, df in raw.items()}, macro_df=macro_df, cot_df=cot_df
    )
    truncated = {a: df.iloc[:-TRUNCATE_BARS].copy() for a, df in raw.items()}
    aligned_trunc, norm_trunc = FeatureEngine().preprocess_data(
        truncated, macro_df=macro_df, cot_df=cot_df
    )
